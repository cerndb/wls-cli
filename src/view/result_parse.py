...
class ResultParser(object):
    ...
    def parse(self, result, is_get_domain_data_op):
        ''' parse interface '''
...
class RESTParser(ResultParser):
    ...
    def __init__(self, data_mockup):
        ''' Constructor '''
        self.data_mockup = data_mockup
        super(RESTParser, self).__init__()
        
        if data_mockup.view_test_file is not None:
            sys.stdout = data_mockup.view_test_file
    
    def parse(self, json_result, is_get_domain_data_op):
        ''' parse json output'''
        try:
            if not is_get_domain_data_op:
    ...
    def print_result(self, json_result):
        # if user wants parsed output
        if not (self.data_mockup.raw):
            self.print_parsed_json(json_result, self.data_mockup.long)
                   
        # user wants to print default json output
        else:
            self.print_default_json(json_result)
        
    def print_parsed_json(self, json_result, long):
        ...
        len(filter(lambda x: "/management/wls/latest/deployments/application" in \
                    x["uri"], json_result["links"])) > 0 :
            self.print_app(json_result, long)
        ...
        len(filter(lambda x: "/management/wls/latest/deployments/library" in \
                    x["uri"], json_result["links"])) > 0) :
            self.print_show_deployments(json_result, long)
            
        # show jobs
        elif len(filter(lambda x: "/management/wls/latest/jobs" in \
                    x["uri"], json_result["links"])) > 0:
            self.print_jobs(json_result, long)
                     
        # show: targets
        elif json_result.has_key("items") and (len(json_result["items"]) > 0 and \
        json_result["items"][0].has_key("type")):
            self.print_show_targets(json_result)
    ...
    def print_jobs(self, json_result, long):
        if (long == True):
            ...
            parameters_to_omit = ["name", "beginTime", "endTime", 
                                  ...
                                  "type"]
        #parameters_to_omit = ["name"]
        if not self.data_mockup.servlets: parameters_to_omit.append("servlets")
        for x in json_result["items"]:
            print "\n%s" % x["name"]
            if long or self.data_mockup.servlets:
    ...
    def print_show_deployments(self, json_result, long):
    ...
    def print_show_targets(self, json_result):
        ''' printing 'show' operation '''
        parameters_to_omit = ["name"]
        if not self.data_mockup.servlets: parameters_to_omit.append("servlets")
        for x in json_result["items"]:
    ...
    def print_app(self, json_result, long):
